...
namespace Barcode_recognition
{
    ...
    public class InformBarcode:WorkWithDB
    {  public int numberblack (Bitmap image, Color[,] All)
        {
            int numberblack = 0;
                for (int i = 0; i < (image.Width - 1); i++)
            {
                if (All[i, 0].ToArgb() == Color.Black.ToArgb())
                {
                    for (int j = i; j < (image.Width - 1); j++)
                    {
                        if (All[j, 0].ToArgb() == Color.Black.ToArgb())
                        {
       ...
       public string searchProduct(string barcode)
        {
            string result="";
            openDB();
            SQLiteCommand command = new SQLiteCommand("SELECT name FROM Product WHERE barcode='" + barcode + "'", db);
   ...
   public class BarcodeEAN
    {
        int schet = 0;
        string LG = "";
        public string[] barcodeEAN8(Bitmap image, Color[,] All)
        {
            string[] barcodeEAN8 = new string[8];
            schet = 0;
            int[] wight_lines = new int[22];
            LG = "";
            //Считаем ширину каждой черной полосы
            for (int j = 0; j < 22; j++)
            {
                for (int i = schet; i < (image.Width - 1); i++)
                {
                    if (All[i, 0].ToArgb() == Color.Black.ToArgb())
                    {
                        wight_lines[j]++;
                        if (All[i + 1, 0].ToArgb() != Color.Black.ToArgb())
                        {
                            schet = i + 1;
                            break;
                        }
            ...
            int min_line = wight_lines.Min();
            int[] linecolor = new int[67];
            int nomer = 0;
            //узнаем цвет каждой полоски
            for (int i = 0; i < (image.Width - 1); i++)
            {
                if (All[i, 0].ToArgb() == Color.Black.ToArgb())
                {
                    for (int j = i; j < i + 67 * min_line; j = j + min_line)
                    {
                        ...
                        if (All[j, 0].ToArgb() == Color.White.ToArgb())
                        {
                            linecolor[nomer] = 0;
                        }
                        nomer++;
            ...
            int number_2 = 0;
            //кодировка каждого числа в штрих-коде
            for (int i = 3; i < 64; i = i + 7)
            {
                if (i == 31)
                {
                    i = i + 5;//середина
                ...
                for (int j = 0; j < 7; j++)
                {
                    barcodeEAN8[number_2] = barcodeEAN8[number_2] + Convert.ToString(linecolor[i]);
                    i++;
                }
                i = i - 7;
                number_2++;
        ...
        public string[] barcodeEAN13(Bitmap image, Color[,] All)
        {
            //string[] barcodeEAN13 = new string[12];
            string[] barcodeEAN13 = new string[12];
            
            LG = "";
            schet = 0;
            int[] wight_lines = new int[30];
            for (int j = 0; j < 30; j++)
            {
                for (int i = schet; i < (image.Width - 1); i++)
                {
            ...
            int min_line = wight_lines.Min();
            int[] linecolor = new int[95];
            int nomer = 0;
            for (int i = 0; i < (image.Width - 1); i++)
            {
                if (All[i, 0].ToArgb() == Color.Black.ToArgb())
                {
                    for (int j = i; j < i + 95 * min_line; j = j + min_line)
                    {
                        if (All[j, 0].ToArgb() == Color.Black.ToArgb())
                        {
                            linecolor[nomer] = 1;
            ...
            for (int i = 3; i < 92; i = i + 7)
            {
                ...
                for (int j = 0; j < 7; j++)
                {
                    barcodeEAN13[number_2] = barcodeEAN13[number_2] + Convert.ToString(linecolor[i]);
                    i++;
                }
                i = i - 7;                             
                number_2++;
            ...
            result[0]= Convert.ToString(firstNumber); ;
            for (int j = 1; j < 13; j++)
            {
                result[j] += barcodeEAN13[j-1];
            }
            return result;
        }
    ...
    public class BarcodeCode
    {
       public string tip = "";
        ...
        public static void barcodeCode(ref System.Collections.ArrayList CodesRead, Bitmap bmp)
        {   
            int numscans = 100;
